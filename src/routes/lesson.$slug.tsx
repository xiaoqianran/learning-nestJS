...
function LessonPage() {
  ...
  const officialHref = lesson.official
    ? lesson.official.startsWith("http")
      ? lesson.official
      : `https://docs.nestjs.com${lesson.official === "/" ? "" : lesson.official}`
    : null;
        ...
        {officialHref ? (
          <p className="mt-3">
            ...
            <span className="ml-2 font-mono text-[11px] text-subtle">
              docs.nestjs.com{lesson.official === "/" ? "" : lesson.official}
            </span>
          </p>
        ) : null}
      </header>
        ...
        {next ? (
          ...
          >
            <div className="rounded-xl border border-border bg-surface p-4">
              <p className="text-xs text-muted">继续未完成课</p>
              <p className="mt-1 font-medium text-fg">{getContinueLesson(completed).title}</p>
            </div>
          </Link>
        )}
      </nav>
